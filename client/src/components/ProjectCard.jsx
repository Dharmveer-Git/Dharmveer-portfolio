  ...
  return (
    <article className="project">
      <div className={`project-art tone-${index % 3}`}>
        {project.image ? <img src={project.image} alt={`${project.title} screenshot`} loading="lazy" /> : null}
      ...
      <div className="project-copy">
        <p className="kicker">{project.subtitle || 'Full-stack project'}</p>
        <h3>{project.title}</h3>
        <p className="project-summary">{project.description}</p>
        <ul aria-label="Technologies">{project.tags?.map((tag) => <li key={tag}>{tag}</li>)}</ul>
        <div className="project-links">
          <Link className="details-link" to={`/projects/${slugify(project.title)}`} aria-label={`View details for ${project.title}`}>View Details <Arrow /></Link>
